...
# Quick and Dirty!!!
media_css={ 'all': ( 'http://cistrome.org/dc/static/jquery.autocomplete.css', ) }
media_js=( 'http://cistrome.org/dc/static/admin/js/jquery.autocomplete.js', )
	...
	def render( self, name, value, *args, **kwargs ):
		...
		if rel_to in self.admin_site._registry: # If the related object has an admin interface:
			...
			output.append( u'<a href="%sadd/" class="add-another" id="add_id_%s" onclick="return showAutocompletePopup(this);"> '%\
				( related_url, name ) )
			output.append( u'<img src="%sadmin/img/icon_addlink.gif" width="10" height="10" alt="%s"/></a>'%( settings.ADMIN_MEDIA_PREFIX, _( 'Add Another' ) ) )
